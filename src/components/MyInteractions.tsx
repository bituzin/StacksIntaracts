...
export default function MyInteractions({ stxAddress, network, onBack }: MyInteractionsProps) {
  ...
  return (
    <div style={{ maxWidth: 400, margin: '32px auto', background: 'var(--bg-card)', borderRadius: 10, padding: 24, boxShadow: '0 4px 24px rgba(0,0,0,0.2)' }}>
      {!showGM && !showPostMsg && (
        <>
          <h3 style={{ marginTop: 0, color: 'var(--accent)', textAlign: 'center' }}>My Interactions</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginTop: 24 }}>
            <button className="wallet-button" onClick={() => setShowGM(true)}>GM stats</button>
            <button className="wallet-button" onClick={() => setShowPostMsg(true)}>Post Message stats</button>
      ...
      {showGM && (
        <>
          ...
          <div style={{ textAlign: 'center', marginTop: 24 }}>
            <button className="wallet-button" onClick={() => setShowGM(false)}>Back</button>
          </div>
        </>
      )}
      {showPostMsg && (
        <>
          ...
          <div style={{ textAlign: 'center', marginTop: 24 }}>
            <button className="wallet-button" onClick={() => setShowPostMsg(false)}>Back</button>
          </div>
        </>
      )}
    </div>
